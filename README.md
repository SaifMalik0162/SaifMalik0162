# Hi! I'm < Saif Malik />

**Full-Stack Developer | Systems Builder | Product Engineer**

I’m a Canada-based developer focused on building real-world systems — from mobile and web apps to backend services and self-hosted infrastructure. I design end-to-end projects that bring apps, APIs, and data together into complete platforms.

My work spans app development, API integration, and infrastructure, with a focus on making products functional, stable, and real.

---

## 🧭 About Me

- 🌍 Based in Canada  
- 🏗️ Full-stack systems builder  
- 🔧 Deep into servers, storage, networking & containers  
- 🤝 Open to collaboration
- 💬 Ask me about infrastructure, or system design  

---

## 🔥 Currently Building

🚗 **Empire Connect** — a full-stack platform for real car culture,
---

## 🛠️ Skills & Technologies

### Languages & App Development  
![Swift](https://img.shields.io/badge/Swift-orange?style=for-the-badge&logo=swift&logoColor=white)
![SwiftUI](https://img.shields.io/badge/SwiftUI-blue?style=for-the-badge&logo=apple&logoColor=white)
![Java](https://img.shields.io/badge/Java-red?style=for-the-badge&logo=oracle&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=for-the-badge&logo=javascript)
![REST](https://img.shields.io/badge/REST_API-005571?style=for-the-badge)

### Backend & Databases  
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![Postgres](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql)
![Firebase](https://img.shields.io/badge/Firebase-ffca28?style=for-the-badge&logo=firebase&logoColor=black)
![Supabase](https://img.shields.io/badge/Supabase-3ecf8e?style=for-the-badge&logo=supabase&logoColor=white)

### Infrastructure & DevOps  
![Linux](https://img.shields.io/badge/Linux-black?style=for-the-badge&logo=linux)
![Ubuntu](https://img.shields.io/badge/Ubuntu-orange?style=for-the-badge&logo=ubuntu&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ed?style=for-the-badge&logo=docker&logoColor=white)
![Docker Swarm](https://img.shields.io/badge/Docker_Swarm-2496ed?style=for-the-badge)
![TrueNAS](https://img.shields.io/badge/TrueNAS-0095d5?style=for-the-badge)
![ZFS](https://img.shields.io/badge/ZFS-cc0000?style=for-the-badge)
![Cloudflare](https://img.shields.io/badge/Cloudflare-f38020?style=for-the-badge&logo=cloudflare&logoColor=white)
![NGINX](https://img.shields.io/badge/NGINX-009639?style=for-the-badge&logo=nginx)

### Tools & Workflow  
![Git](https://img.shields.io/badge/Git-f05032?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github)
![Networking](https://img.shields.io/badge/Networking-blue?style=for-the-badge)

---

## 💼 Experience

I build **full-stack platforms** that run in the real world — from mobile apps backed by APIs, to containerized services running on private infrastructure.

My projects include:
- User-scoped data systems  
- Image & media pipelines  
- Authentication & account systems  
- Self-hosted cloud platforms  
- Mobile + backend ecosystems  

All designed with **persistence, security, and scalability** in mind.

---

## 🚀 How I Work

I don’t separate “frontend” and “backend” — I build **systems**.  
UI, API, storage, networking, and deployment all matter. I care about what still works **six months later**, not just on launch day.

---

## 🌐 Socials

[![GitHub](https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github)](https://github.com/SaifMalik0162)

---

## 📊 GitHub Stats

![Saif's GitHub stats](https://github-readme-stats.vercel.app/api?username=SaifMalik0162&show_icons=true&theme=tokyonight)  
![GitHub Streak](https://streak-stats.demolab.com/?user=SaifMalik0162&theme=tokyonight)

---

## 🌟 Let’s Build

Always open to ambitious projects, startups, and open-source systems.

If it involves apps, servers, or clean UI, I’m in.

**Turning ideas into real things.**
